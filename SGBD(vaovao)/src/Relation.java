...
public class Relation {
    ...
    private boolean verifierCondition(Object val1, Object val2, String operateur) {
        // Vérifier si les deux valeurs sont nulles
        if (val1 == null || val2 == null) {
            if ("=".equals(operateur)) {
                ...
                throw new IllegalArgumentException("Impossible de comparer des valeurs nulles avec l'opérateur: " + operateur);
            }
        }

        // Vérifier si les deux valeurs sont des dates
        if (val1 instanceof Date && val2 instanceof Date) {
            ...
            switch (operateur) {
                ...
                case "<=":
                    return !date1.after(date2);
                default:
                    throw new IllegalArgumentException("Opérateur non reconnu pour Date: " + operateur);
            }
        }

        // Vérifier si les deux valeurs sont des nombres
        if (val1 instanceof Number && val2 instanceof Number) {
            ...
            switch (operateur) {
                ...
                case "<=":
                    return num1 <= num2;
                default:
                    throw new IllegalArgumentException("Opérateur non reconnu pour Number: " + operateur);
            }
        }

        // Vérifier si les deux valeurs sont des chaînes de caractères (String)
        if (val1 instanceof String && val2 instanceof String) {
            ...
            switch (operateur) {
                ...
                case "<=":
                    return str1.compareTo(str2) <= 0;
                default:
                    throw new IllegalArgumentException("Opérateur non reconnu pour String: " + operateur);
            }
        }
        ...
        if (val1 instanceof Comparable && val2 instanceof Comparable) {
            ...
            switch (operateur) {
                ...
                case "<=":
                    return comp1.compareTo(comp2) <= 0;
                default:
                    throw new IllegalArgumentException("Opérateur non reconnu pour Comparable: " + operateur);
            }
        }

        // Si le type des objets est inconnu
        throw new IllegalArgumentException("Types incompatibles ou non supportés: " + val1.getClass() + " et " + val2.getClass());
    }
    ...
    public Relation jointureExterne(Relation relation1, Relation relation2, String condition, boolean gauche, boolean droite) throws Exception {
        // Ajout des noms des relations comme préfixes pour chaque attribut
        int i = 0;
        ...
        // Création de la relation résultante avec les attributs combinés
        ...
        // Analyse de la condition pour extraire l'opérateur et les attributs concernés
        ...
        if (index1 == -1 || index2 == -1) {
            throw new Exception("L'un des attributs spécifiés dans la condition est introuvable.");
        }

        // Tableau pour marquer les tuples de la deuxième relation qui ont été appariés
        boolean[] matchRel2 = new boolean[relation2.valeurs.length];

        // Parcourir les tuples de la première relation
        for (Object[] tuple1 : relation1.valeurs) {
            boolean match = false;

            // Parcourir les tuples de la deuxième relation
            for (int j = 0; j < relation2.valeurs.length; j++) {
                Object[] tuple2 = relation2.valeurs[j];

                // Comparer les valeurs des attributs spécifiés dans la condition
                if (verifierCondition(tuple1[index1], tuple2[index2], operateur)) {
                    // Ajouter le tuple fusionné au résultat
                    resultat.insererDonnees(concatTuples(tuple1, tuple2));
                    match = true;
                    matchRel2[j] = true; // Marquer ce tuple comme apparié
                }
            }

            // Ajouter les tuples de gauche non appariés
            if (!match && gauche) {
    ...
    public String toString() {
        ...
        if (nombreDeLignes == 0) {
            sb.append("(Aucune donnée dans cette relation)");
        }
